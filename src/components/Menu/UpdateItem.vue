...
export default {
  ...
  methods: {
    async UpdateItem() {
      ...
      if (!this.v$.$error) {
        const UpdateItemResult = await axios.put(
        ...
        if (UpdateItemResult.status == 200) {
          this.successUserMessage = "Update Item Successfully!";
    ...
    async canThisUserAccessToItem() {
      ...
      if (result.status == 200) {
        this.listOfItems = result.data[0];
        this.itemName = this.listOfItems.name;
        this.itemPrice = this.listOfItems.price;
        this.itemQuant = this.listOfItems.quant;
        this.itemDesc = this.listOfItems.description;
        this.pickedCategory = this.listOfItems.catId;
